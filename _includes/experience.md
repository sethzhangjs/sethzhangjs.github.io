## Industry Experience

<table frame="void" rules="none" style="width: 100%; border-collapse: collapse;">
    <tr style="border: none;">
        <td style="width: 15%; border: none; text-align: center; vertical-align: middle; padding: 12px 0;">
            <img src="assets/img/emp/visilant.png" style="width: 50px; height: 50px; object-fit: contain; border-radius: 6px;">
        </td>
        <td style="border: none; vertical-align: middle; padding: 12px 0;">
            <div>
                <b>Visilant</b>
                <br>
                Feb. 2026 – Present &nbsp;·&nbsp; Baltimore, MD
                <br>
                Machine Learning Engineer &nbsp;·&nbsp; Part-time
            </div>
        </td>
    </tr>
    <tr style="border: none;">
        <td style="width: 15%; border: none; text-align: center; vertical-align: middle; padding: 12px 0;">
            <img src="assets/img/emp/bosch.png" style="width: 50px; height: 50px; object-fit: contain; border-radius: 6px;">
        </td>
        <td style="border: none; vertical-align: middle; padding: 12px 0;">
            <div>
                <b>Bosch</b>
                <br>
                Nov. 2023 – Mar. 2024 &nbsp;·&nbsp; Suzhou, China
                <br>
                Software Engineer &nbsp;·&nbsp; Internship
            </div>
        </td>
    </tr>
</table>
